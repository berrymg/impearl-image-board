/**
 * @fileoverview
 * Image board for Impearl — threads + replies (integrated).
 */
...
imib.util = {
  boardId() {
    ...
    return (el.dataset && el.dataset.boardId) ? el.dataset.boardId : "board";
  },
  tsToString(ts) {
    try {
  ...
  snippet(s, n = 180) {
    s = s || ""; return s.length > n ? s.slice(0, n) + "…" : s;
  },
  getFileInput() {
  ...
  applyAuthVisibility() {
    ...
    document.querySelectorAll('.editSpan, .deleteSpan').forEach(el => {
      el.style.display = loggedIn ? '' : 'none';
    });
  }
};

/** Model (Firestore + Storage) */
imib.BoardModel = class {
  ...
  _boardRef() {
    return this.db.collection(imib.FB_COLLECTION_BOARDS).doc(this.boardId);
  }
  _threadsCol() {
    return this._boardRef().collection(imib.FB_COLLECTION_THREADS);
  }
  _postsCol(threadId) {
    return this._threadsCol().doc(threadId).collection(imib.FB_COLLECTION_POSTS);
  }
  ...
  /** Create thread + OP atomically */
  async createThread({ subject, comment, file }) {
  ...
  /** Live watch: stickies first, then newest-bumped */
  watchThreads(onChange) {
    return this._threadsCol()
      ...
      .onSnapshot((snap) => {
        ...
        // keep stickies on top client-side (optional double safety)
        arr.sort((a, b) => {
          const s = (b.isSticky ? 1 : 0) - (a.isSticky ? 1 : 0);
          if (s) return s;
          const ta = a.lastBumpAt?.seconds || 0;
          const tb = b.lastBumpAt?.seconds || 0;
          return tb - ta;
        });
        onChange(arr);
      }, (err) => {
  ...
  /** ---- REPLIES: create + live watch ---- */

  /**
   * Create a reply under a thread (optional file).
   * @param {Object} opts
   * @param {string} opts.threadId
   * @param {string|null} opts.replyToPostId
   * @param {string} opts.subject
   * @param {string} opts.comment
   * @param {File|null} opts.file
   * @param {boolean} opts.sage - if true, do not bump thread
   */
  async createReply({ threadId, replyToPostId = null, subject = "", comment = "", file = null, sage = false }) {
  ...
  /**
   * Live watch replies for a thread (excludes OP).
   * @param {string} threadId
   * @param {(posts: Array<Object>) => void} onChange
  ...
  watchReplies(threadId, onChange) {
    return this._postsCol(threadId)
      ...
      .onSnapshot(
        ...
        (err) => {
          console.error("watchReplies error:", err);
        }
      );
  }
  ...
  async deletePost({ threadId, postId, isOP }) {
    ...
    if (!isOP) {
      // delete a single reply, update counters, and remove its image if present
      const postRef   = this._postsCol(threadId).doc(postId);
      const threadRef = this._threadsCol().doc(threadId);
      const dec = firebase.firestore.FieldValue.increment(-1);
  
      // grab image path (if any) before delete
      ...
      // best-effort storage delete (won't block UI)
      try {
        if (storagePath && this.storage) await this.storage.ref(storagePath).delete();
      } catch (_) {}
      return;
    }
  
    // OP delete: wipe entire thread (posts + thread). Chunk to respect batch limits.
    const postsSnap = await this._postsCol(threadId).get();
    const allDocs = [];
    postsSnap.forEach(d => allDocs.push(d));
  
    // Delete images (best-effort) while we iterate
    if (this.storage) {
    ...
    // chunk deletes (max ~400 ops per batch to be safe)
    while (allDocs.length) {
      const chunk = allDocs.splice(0, 400);
      const batch = this.db.batch();
      chunk.forEach(d => batch.delete(d.ref));
      // only delete the thread doc in the final pass
...
imib.BoardController = class {
  constructor(model) {
    ...
    // --- replies wiring state ---
    this._watchedReplies = new Map(); // threadId -> unsubscribe
    this._replyTarget = { threadId: null, replyToPostId: null };

    // Reply modal: capture thread + post target from the clicked [reply]
    $("#replyModal").on("show.bs.modal", (e) => {
      ...
      this._replyTarget.replyToPostId = $(trigger).data("reply-to") || $thread.find(".op").data("post-id") || null;
    ...
    $("#replyModal .btn.btn-primary").on("click", async (evt) => {
      ...
      try {
        await this.model.createReply({
          ...
          subject, comment, file, sage: false,
        });
    ...
    // Observe #threadContainer for re-renders to (lazily) attach reply watchers
...
/* EDIT modal: capture target + prefill */
$("#editModal").on("show.bs.modal", (e) => {
  const trigger  = e.relatedTarget;           // <a> that opened it
  const $thread  = $(trigger).closest(".thread");
  const threadId = $thread.data("thread-id");
  const postId   = trigger.getAttribute("data-edit");
  const isOP     = $(trigger).closest(".op").length > 0;
...
/* DELETE modal: capture target */
$("#deleteModal").on("show.bs.modal", (e) => {
  const trigger  = e.relatedTarget;
  const $thread  = $(trigger).closest(".thread");
  const threadId = $thread.data("thread-id");
  const postId   = trigger.getAttribute("data-delete");
  const isOP     = $(trigger).closest(".op").length > 0;
...
$("#deleteModal .btn.btn-primary").on("click", async (evt) => {
  ...
  _prefillEditModal(threadId, postId, isOP) {
    ...
    if (isOP) {
      ...
      $post = $thread.find(`.reply[data-post-id="${CSS.escape(postId)}"]`);
    }
  ...
  async submitPost() {
    ...
    if (!subject && !comment && !file) {
      alert("You must have at least a title OR a comment");
      return;
    }

    await this.model.createThread({ subject, comment, file });
  ...
  updateView(threads) {
    // render into #threadContainer (exists in your HTML)
    const cont = document.getElementById("threadContainer");
    if (!cont) return;

    cont.innerHTML = threads.map((t) => {
      const ts = imib.util.tsToString(t.createdAt);
      const id = t.id;
      ...
      return `
        <div class="thread" id="thread-${id}" data-thread-id="${id}">
          <div class="op" data-post-id="${imib.util.esc(t.opPostId || "")}">
            ${imgHtml}
            <div class="post-bar">
              <span class="author-span">anonymous</span>
  ...
  _resetReplyWatches() {
    // unsubscribe old listeners so fresh ones will immediately fire
